...
if(G>1){
  M_exp <- array(NA_real_, dim = c(G, n, n),
                 dimnames = list(sex = levels(d$f_sex),
                                 from = c(unique(d$t_l),unique(d$t_l)+5,11),
                                 to = c(unique(d$t_l),unique(d$t_l)+5,11)))
}else{
  M_exp <- array(NA_real_, dim = c(G, n, n),
                 dimnames = list(sex = "Combined",
                                 from = c(unique(d$t_l),unique(d$t_l)+5,11),
                                 to = c(unique(d$t_l),unique(d$t_l)+5,11)))
}
