...
sex_loc_ttt_delTime_model <- function(parms) {
  ...
  for(g in 1:G){
    for (t in seq_len(s)) {
      ...
      for(loc in 1:4){
        for(loc2 in (loc+1):5){
          T_mat[g,loc,loc2,t] <- exp(t_vars[g,icnt,1])
          icnt <- icnt + 1
        }
  ...
  for(g in 1:G){
    for (t in seq_len(s)) {
      ...
      S__[g,,,t] <- as.matrix(Matrix::expm(S[g,,,t]))
      S[g,,,t] <- as.matrix(Matrix::expm(S[g,,,t] * delT[s]))
    }
  }
  RTMB::REPORT(S)
  RTMB::REPORT(S__)
  ...
  for(g in 1:G){
    for (t in 1:s) {
      for(loc in 1:5){
        mu_p <- p[g,loc]
        # mu_p <- p[1]
        D[g,loc,loc,t] <- -exp(mu_p + u_p_gls[g,g_u_p[loc],t] + u_p_gl[g,loc] + u_p_g[g]) * surv[t,loc]
        D[g,loc,loc+5,t] <- -D[g,loc,loc,t]
      }
      D[g,,,t] <- as.matrix(Matrix::expm(D[g,,,t]))
    }
  }
  D_init <- array(0, dim = c(G, 11, 11, s))
  g_u_p <- c(1,2,3,4,5)
  for(g in 1:G){
    for (t in 1:s) {
      for(loc in 1:5){
        mu_p <- p[g,loc]
        # mu_p <- p[1]
        D_init[g,loc,loc,t] <- -exp(mu_p + u_p_gls[g,g_u_p[loc],t] + u_p_gl[g,loc] + u_p_g[g])
        D_init[g,loc,loc+5,t] <- -D_init[g,loc,loc,t]
      }
      D_init[g,,,t] <- as.matrix(Matrix::expm(D_init[g,,,t]))
    }
  }
  RTMB::REPORT(D)
  RTMB::REPORT(u_p_gls)
  ...
  for (i in seq_along(t_k)) {
    ...
    recap_live   <- if (n_loc > 1) as.integer(f_rl[i]) else 1L
    # print(recap_live)
    alpha[idx_live] <- 1

    ## transition kernel for this fish: Initial tagging must consider
    if(entry<s){
      for (t in seq(entry+1, s)){
        Tmat <- S[sex[i],,,t] %*% D[sex[i],,,t]
        alpha <- alpha %*% Tmat
      }
    }else{
...
for(yyy in 2:12){
  ...
  d <- f %>%
    ...
    mutate(tmp_y = as.integer(lubridate::year(as.Date(Date_Cap)))) %>%
    filter(!(tmp_y == Return_Yr & lubridate::yday(as.Date(Date_Cap))<210)) %>% #Remove
    mutate(t_k = as.integer(as.Date(Date_Cap) - min(as.Date(Date_Cap))),
         r_k = as.integer(as.Date(Date_Recap) - min(as.Date(Date_Cap))),
         t_yr = Return_Yr) %>%
    ...
    mutate(f_rk = factor(r_k, levels = unique(sort(as.numeric(t_k))))) %>%
    ...
    group_by(t_k,r_k,t_l, r_l, tag, t_yr, f_yr, f_tk, f_rk, f_rl, f_tl, f_sex, p_f_sex) %>%
    summarise(n = n()) %>%
    filter((t_l<=r_l) | is.na(r_l))

  s <- max(as.integer(d$f_tk))
  G <- 2

  # Counts per period
  u_vec <- numeric(s)
  u_sum <- aggregate(list(n = d$n), by = list(t = d$f_tk), sum)
  ...
  surv <- table(c(d$f_tk,d$f_rk),c(d$f_tl,d$f_rl))
  surv[surv>0] <- 1
  delT <- c(0,diff(sort(unique(d$t_k))))


  data <- list(
    t_k               = as.integer(d$f_tk),
    r_k               = as.integer(d$f_rk),
    t_l               = d$t_l,
  ...
  params <- list(u_phi_gls = array(0,c(data$G,5,data$s)),
                 ...
                 log_sd_p = (-1),
                 u_w = array(0,c(1,1,data$s)),
                 log_sd_w = (-1),
                 # phi = array(-0.5,c(data$G,5)),
                 phi = -1,
                 p = array(-0.5,c(data$G,5)),
                 t_vars = array(0, c(data$G,5*(5-1)/2,1)),
                 # t_vars = array(0, c(1,5*(5-1)/2),1),
                 log_sd_t_vars = -1
  ...
  sex_loc_ttt_delTime_obj <- MakeADFun(sex_loc_ttt_delTime_model,
                       data = data,
  ...
  opt <- nlminb(sex_loc_ttt_delTime_obj$par, sex_loc_ttt_delTime_obj$fn, sex_loc_ttt_delTime_obj$gr)
  rep <- sex_loc_ttt_delTime_obj$report()
  sdr <- sdreport(sex_loc_ttt_delTime_obj)
  my_rep[[yyy]] <- rep
...
saveRDS(out,"output/sex_loc_ttt_delTime.rds")
#
for(i in 1:length(out$rep)){
    print(paste(i,
                round(sum(out$rep[[i]]$Nsuper_vec)),
                round(sum(out$rep[[i]]$Nsuper_vec - 1.96 * out$sd[[i]]$sd$Nsuper_vec)),
                round(sum(out$rep[[i]]$Nsuper_vec + 1.96 * out$sd[[i]]$sd$Nsuper_vec)),
          ",", dans$est[dans$Year%in%years[i]]))
  }
#
#
...
# Your matrix
mat <- round(out$rep[[2]]$S__[2, 1:5, 1:5, 10], 3)

# Viridis color palette
pal <- viridis::inferno(200)
mat[lower.tri(mat)] <- min(mat[upper.tri(mat)])

# Make corrplot with diagonal and upper triangle
corrplot(
  as.matrix(mat),
  method       = "color",      # fill full square
  type         = "upper",      # upper triangle + diagonal
  diag         = TRUE,         # keep diagonal
  tl.col       = "black",      # axis label color
  addgrid.col  = "white",      # grid lines
  col          = pal,          # viridis colors
  col.lim      = c(min(mat[upper.tri(mat)]), max(mat)),      # color scale 0–1
  cl.pos       = "r",          # color legend on right
  cl.cex       = 0.8,          # color legend size
  number.cex   = 0.75,         # number font size
  addCoef.col  = "black"       # show values in boxes
)
